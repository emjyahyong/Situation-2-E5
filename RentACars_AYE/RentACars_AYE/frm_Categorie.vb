...
Public Class frm_Categorie
    ...
    Private Sub ChangerCategorie(valeur As Integer)
        ...
        'Exécution de la requête SQL sur la connexion en cours
        If valeur = 1 Then
            sdaCategorie.SelectCommand = New MySqlCommand("Select * From categorie Inner Join utilitaire On categorie.code = utilitaire.code Where estReserve = 0", cnxBD)
        ElseIf valeur = 2 Then
            'sdaCategorie.SelectCommand = New MySqlCommand("Select * From CATEGORIE Inner Join TOURISME On CATEGORIE.CODE = TOURISME.CODE Inner Join UTILITAIRE On CATEGORIE.CODE = UTILITAIRE.CODE Where ESTRESERVE = 1", cnxBD)
            sdaCategorie.SelectCommand = New MySqlCommand("Select * From categorie  Where estReserve = 1", cnxBD)
        Else
            sdaCategorie.SelectCommand = New MySqlCommand("Select * From categorie Inner Join tourisme On categorie.code = tourisme.code Where estReserve = 0", cnxBD)
        End If
        'Récupération des données
        sdaCategorie.Fill(dtCategorie)
        'Affectation du jeu d'enregistrement résultat à la grille de données
